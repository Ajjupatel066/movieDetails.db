...
app.get("/directors/", async (request, response) => {
  const getAllTheDirectorsQuery = `
    SELECT *
    FROM director;`;
  const movieArray = await db.all(getAllTheDirectorsQuery);
  response.send(
    movieArray.map((director) => convertDirectorDetailsPascalCase(director))
  );
});

//7. Returns a movie names directed by a specified director
